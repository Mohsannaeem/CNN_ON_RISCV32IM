...
)
{
	...
	int32_t* y = (int32_t*)malloc(m_row * m_col* sizeof(int32_t));//cal
	size_row = m_row;
	size_col = m_col;
			...
			for (unsigned int k1 = 0; k1 < row; ++k1)
			{
				for (unsigned int k2 = 0; k2 < col; ++k2)
				{
					...
					if (j - st_y + k2 > max_imr)
						continue;
					int16_t weight = *(arr + (d * m_depth * row * col) + (e * row * col) + (row * (k1_size - k1 - 1)) + (k2_size - k2 - 1));//Convolution input 1x28x28
					int16_t input = (*(im + e * m_row * m_col + (i - st_x + k1) * m_row + (j - st_y + k2))); //Convolution Filter or kernal 3x3
